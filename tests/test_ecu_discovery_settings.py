...
def test_ecu_discovery_scan_interval_setting():
    """Verify that scan_interval setting exists and affects passive_scan and signal_summary."""
    device = Device("virtual", None, "test_channel", 250000)
    try:
        ...
        # We patch 'libs.pretty_shim' because that is how ecu_discovery imports it.
        with patch("libs.pretty_shim.PRETTY_AVAILABLE", True):
            with patch("truckdevil.modules.ecu_discovery.time.sleep") as mock_sleep:
