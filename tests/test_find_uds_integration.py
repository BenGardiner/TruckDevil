            ...
            if msg:
                ...
                if (msg.arbitration_id >> 16) & 0xFF == 0xDA:
                    ...
                    if len(data) >= 2 and (data[1] == 0x3E or data[1] == 0x10):
                        ...
                        resp_msg = can.Message(arbitration_id=resp_id, data=[0x02, data[1] + 0x40, 0x00], is_extended_id=True)
                        bus.send(resp_msg)
    except Exception:
        pass
    finally:
...
def test_find_uds_success(virtual_device_shared_channel):
            ...
            if msg:
                ...
                if (msg.arbitration_id >> 16) & 0xFF == 0xDA:
                    ...
                    if len(data) >= 2 and (data[1] == 0x3E or data[1] == 0x10):
                        ...
                        # PCI 03
                        resp_msg = can.Message(arbitration_id=resp_id, data=[0x03, 0x7F, data[1], 0x11], is_extended_id=True)
                        bus.send(resp_msg)
    except Exception:
        pass
    finally:
...
def test_find_uds_nr_success(virtual_device_shared_channel):
    device, channel = virtual_device_shared_channel
